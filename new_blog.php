...
<div class="container">
  ...
  <div class="row">
	...
	<?php 
				...
				while($article = mysql_fetch_array($article_set))
				{
				  echo '<div class="card blue-grey darken-1">';
					echo '<div class="card-content white-text">';
					  if($sel_genre != 'batch')
					  {
						echo '<span class="card-title">' . $article['title'] . '</span><div class="right" style="color:grey">By: ' . $article['article_by'] . '</div>';
						echo '<p>' . substr((nl2br($article['content'])), 0, 400) . '</p>';
					  }
					  else
					  {
						echo '<span class="card-title">' . $article['title'] . '</span>';
						echo '<p>' . substr((nl2br($article['content']))). '</p>';
					  }
					echo '</div>';
					echo '<div class="card-action">';
					  echo '<a href="view_blog.php" class="right">Read More</a>';
				   echo '</div>';
				  echo '</div>';
	 ...
	 <?php
		if(isset($_GET['add'])){
			echo '<h5 class="green-text text-lighten-1">Thanks your article has been submitted, it will soon be reviewed and then posted.</h5>';
		}
		if(!isset($_GET['genre'])){
			echo '<div class="row">';
